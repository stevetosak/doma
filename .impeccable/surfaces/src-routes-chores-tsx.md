...
Bumped every primary `bg-rust` button (Add chore, Done, Save/submit in the ChoreForm sheet)
and the secondary Skip button from `py-1`/`py-2` to `py-3`, and the bottom-bar/spine nav tabs
in TabSpine to `py-3` — all now real 44px tap targets. See The Forty-Four-Pixel Rule in
DESIGN.md. Pure padding change; no layout or copy affected.
